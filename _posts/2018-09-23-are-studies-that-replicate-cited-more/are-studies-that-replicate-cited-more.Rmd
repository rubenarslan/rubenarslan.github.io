...
MASTER %>% filter(!is.na(T_pval_USE..R.)) %>% 
  ggplot(aes(T_pval_USE..R.< .05, Citation.count..paper..O.)) + 
...
MASTER %>% filter(!is.na(T_pval_USE..R.)) %>% 
  glm(Citation.count..paper..O. ~ T_pval_USE..R.< .05, data = ., family = quasipoisson()) %>% 
  summary()
```


## Does replication predict 2018 citation counts?
I used the Crossref API to the current citation counts for the papers contained in the RPP. 
Again, there was no association with replication status.

<aside>
__Details__: I first got DOIs, which were missing from the RPP data, by searching Crossref on titles, authors, date. I didn't do any checking that matches were proper. In a next step, I got up-to-date citation counts from CrossRef.
</aside>

```{r layout='l-body-outset'}
MASTER %>% filter(!is.na(T_pval_USE..R.)) %>% 
  ggplot(aes(T_pval_USE..R.< .05, citation_count_2018)) + 
  xlab("Significant replication effect (p < .05)") +
...
<aside>
The correlation between 2018 and 2015 counts is `r round(cor(MASTER$citation_count_2018, MASTER$Citation.count..paper..O.), 2)`. The mean for citations in 2018 is `r round(mean(MASTER$citation_count_2018),2)`, in the RPP data it was `r round(mean(MASTER$Citation.count..paper..O.),2)`. How can citations go DOWN? The citation count in the RPP probably includes more sources, leading to the mean being higher, but these sources don't seem to be systematically different, leading to the maintained rank order. d
...
```{r layout='l-body-outset'}
lm(citation_count_2018 ~ Citation.count..paper..O., data = MASTER)
MASTER <- MASTER %>% mutate(
  citations_after_2018 = citation_count_2018 - Citation.count..paper..O. * 0.87,
  citations_after_2018 = citations_after_2018 - min(citations_after_2018, na.rm = T)
)
MASTER %>% filter(!is.na(T_pval_USE..R.)) %>% ggplot(aes(T_pval_USE..R.< .05, citations_after_2018)) + 
  xlab("Significant replication effect (p < .05)") +
  ylab("Citations after 2015)") +
  geom_beeswarm(alpha = 0.3) + 
  geom_pointrange(stat='summary', fun.data = 'median_hilow',color ='#4F94B0')

MASTER %>% filter(!is.na(T_pval_USE..R.)) %>% glm(citations_after_2018 ~ T_pval_USE..R.< .05, data = ., family = 'quasipoisson') %>% summary()
```
...
MASTER2 <- MASTER %>% 
                ...
                select(Authors = Authors..O., Title = Study.Title..O., DOI, replicated_p_lt_05, citations_2015 = Citation.count..paper..O., citations_2018 = citation_count_2018, citations_after_2018)
library(rbokeh)
figure(width = 1100, height = 500) %>%
  ly_points(citations_2015, citations_after_2018, data = MASTER2,
...
DT::datatable(MASTER2 %>% select(-citations_after_2018) %>% arrange(desc(citations_2018)), escape = F,
...
Did I get the right DOIs? There are probably still some mismatches. Titles are not exactly equal for `r sum(MASTER$mismatch>0)` studies,
but on manual inspection this is only because Crossref separates out the subtitle.
