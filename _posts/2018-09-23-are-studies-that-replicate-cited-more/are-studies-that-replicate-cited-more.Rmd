...
library(rscopus)
	...
	for (i in 1:nrow(MASTER)) {
		tryCatch({
		  que <- generic_elsevier_api(api_key = scopus_api_key, 
		                              query = paste0("DOI(",MASTER$DOI[i],")"),
		                               type = "citation-count",
		                              search_type = "scopus")
		  que <- scopus_search(api_key = scopus_api_key, 
		                              query = paste0("DOI(",MASTER$DOI[i],")"),
		                               type = "citations")
		  que <- abstract_retrieval(api_key = scopus_api_key, 
		                              query = paste0("DOI(",MASTER$DOI[i],")"),
		                               type = "citations")
		  entries_to_df2(que$entries)
			MASTER$citation_count_2018[i] = rcrossref::cr_citation_count(MASTER$DOI[i])
		}, error = function(e) warning(e))
	}
```
