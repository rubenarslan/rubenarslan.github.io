...
MASTER_clean <- MASTER %>% 
                ...
                select(Authors = Authors..O., Title = Study.Title..O., Journal = Journal..O., DOI, replicated_p_lt_05, citations_2015 = Citation.count..paper..O., citations_2018 = citation_count_2018, citations_after_2018, doi)
```
...
We found `r sum(!is.na(MASTER$DOI))` DOIs, so we had DOIs for all our `r nrow(MASTER)` studies^[Were they they all correct? See Appendix].
...
ggplot(data = MASTER_clean, mapping = aes(x = cit_before_2015_scopus, y = cit_after_2015_scopus, color = replicated_p_lt_05)) + 
  geom_point() + geom_smooth(method = lm) + ggtitle("citations before and after 2015") 

ggplot(data = MASTER_clean, mapping = aes(x = cit_scopus_self_ex_before_2015, y = cit_scopus_self_ex_after_2015, color = replicated_p_lt_05)) + 
  geom_point() + geom_smooth(method = lm) + ggtitle("citations (self excluding) before and after 2015") 

ggplot(data = MASTER_clean, mapping = aes(x = self_cites_before_2015, y = self_cites_after_2015, color = replicated_p_lt_05)) + 
  geom_point() + geom_smooth(method = lm) + ggtitle("self citations before and after 2015") 

ggplot(data = MASTER_clean, mapping = aes(replicated_p_lt_05, self_cites_after_2015)) + 
  xlab("Significant replication effect (p < .05)") +
  ylab("Self cites after 2015 (Scopus)") +
  geom_beeswarm(alpha = 0.3) + 
  geom_pointrange(stat='summary', fun.data = 'median_hilow',color ='#4F94B0') + 
  ggtitle("Self citations after 2015 wrt replication") 

ggplot(data = MASTER_clean, mapping = aes(replicated_p_lt_05, cit_before_2015_scopus)) + 
  xlab("Significant replication effect (p < .05)") +
  ylab("Citations before 2015 (Scopus)") +
  geom_beeswarm(alpha = 0.3) + 
  geom_pointrange(stat='summary', fun.data = 'median_hilow',color ='#4F94B0') + 
  ggtitle("Citations before 2015 wrt replication")  

ggplot(data = MASTER_clean, mapping = aes(replicated_p_lt_05, cit_after_2015_scopus)) + 
  xlab("Significant replication effect (p < .05)") +
  ylab("Citations after 2015 (Scopus)") +
  geom_beeswarm(alpha = 0.3) + 
  geom_pointrange(stat='summary', fun.data = 'median_hilow',color ='#4F94B0') + 
  ggtitle("Citations after 2015 wrt replication")  
 

ggplot(data = MASTER_clean, mapping = aes(replicated_p_lt_05, cit_total_scopus)) + 
  xlab("Significant replication effect (p < .05)") +
  ylab("total citation count (Scopus)") +
  geom_beeswarm(alpha = 0.3) + 
  geom_pointrange(stat='summary', fun.data = 'median_hilow',color ='#4F94B0')+ 
  facet_wrap(~ Journal, nrow = 1) + 
  ggtitle("Citations wrt replication per Journal") 
