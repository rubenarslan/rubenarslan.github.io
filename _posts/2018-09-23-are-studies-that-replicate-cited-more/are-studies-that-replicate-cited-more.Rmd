...
s_exclude <- generic_elsevier_api(api_key = api_key,
                          doi = x, query = list(citations = "exclude-self"),
                          date = "2015-2019",
                          type = "citations",
                          search_type = "scopus")
...
MASTER_clean$cit_before_2015_scopus <- as.list(MASTER_clean$doi) %>% 
  map(find_cit_scopus, flag = "prev")

MASTER_clean$cit_after_2015_scopus <- as.list(MASTER_clean$doi) %>% 
  map(find_cit_scopus, flag = "after")

MASTER_clean$cit_total_scopus <- as.list(MASTER_clean$doi) %>% 
  map(find_cit_scopus, flag = "total")

MASTER_clean$cit_total_scopus_self_exclude <- as.list(MASTER_clean$doi) %>% 
