...
m_mixed_2part <- brm(bf(y | cens(ycens) ~ x + (1|gr(id, by = xQ)),
             sigma ~ x), data = days, silent = 2, refresh = 0)
m_2part_level2 <- brm(log(.value) | se(se, sigma = FALSE) ~ x, data = hdis %>% mutate(se = (log(.value)-log(.lower))/2))
```
...
ggplot(draws, aes(y = .variable, x = .value, xmin = .lower, xmax = .upper)) +
  ...
  geom_vline(aes(xintercept = true_value), linetype = 'dashed', 
             data = tibble(model = c("m_between", "m_mixed", "m_mixed_2part", "m_between", "m_mixed", "m_mixed_2part", "m_2part_level2"), .variable = c("b_x", "b_x", "b_x", "b_sigma_x", "b_sigma_x", "b_sigma_x", "b_sigma_x"), true_value = c(b_mean, b_mean, b_mean, b_sd_bs, b_sd_ws, b_sd_ws, b_sd_ws))) +  scale_color_discrete(breaks = rev(levels(draws$model))) +
