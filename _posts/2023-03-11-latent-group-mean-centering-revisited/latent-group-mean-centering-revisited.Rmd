...
In a recent [lecture](https://youtu.be/iwVqiiXYeC4?t=3299), [Richard McElreath](https://twitter.com/rlmcelreath) gives an introduction to Mundlak devices ([code](https://github.com/rmcelreath/stat_rethinking_2023/blob/main/scripts/12_bonus_mundlak.r)). Briefly, social scientists would often like to remove confounding at the group level.^[the group here can be an individual measured several times, a sibling group, a school, a nation, etc.] Econometricians and epidemiologists tend to use fixed effects regression for this. Psychologists often use random effects regressions and adjust for the mean of the predictor of interest. These two solutions yield similar results. [Arguably](https://www.annualreviews.org/doi/full/10.1146/annurev-statistics-040120-024521#_i31), there are more ways to mess up the random effects model, but it's more flexible and efficient and it makes it easier to model e.g., cross-level interactions and effect heterogeneity.

One way you can mess up, is if your group sizes are small (e.g., sibling groups in a low fertility country) and your estimate of the group mean is a poor stand-in for the confounds you'd like to adjust for. A solution to this is to adjust for the latent group mean instead, i.e. to account for the fact that we are estimating it with uncertainty. Doing so^[What McElreath calls Full Luxury Bayes: Latent Mundlak machine] is fairly easy in Stan, but it's less clear how to do it with everyone's favourite Stan interface, brms. [Matti Vuorre](https://twitter.com/vuorre) shared [a solution](https://vuorre.netlify.app/posts/latent-mean-centering/#brms) using brms' non-linear syntax, but I tried it and it doesn't correctly recover simulated effects.^[I think the "problem" is that in the data he used, from McNeish & Hamaker 2020, the group mean varies very little except for measurement error.].

My attempt at a solution:

1. Duplicating the exposure `X->X2`
2. Calculating the SE of the group mean outside the brms call
3. Using the `X2 | mi(Xse)` syntax in brms to estimate the latent group mean with shrinkage. 
4. Adjusting for `mi(X2)` in the regression on `Y`.

This seems to work, but it's a little hacky. I also see no straightforward way to implement within-group centering.

## How did it go? <br><small>High-level summary</small>
- My original plan, using the `me()` syntax did not work consistently, perhaps because there's no shrinkage for the group means.
- My approach does not work for a non-Gaussian exposure `X`, because `mi()` is not implemented for e.g. binary variables.
- I never managed to create a simulated scenario where the latent Mundlak approach consistently outperforms the non-latent Mundlak for `b_X`. It does improve estimates for `b_Xbar` (i.e. the group-level confound), but that's not really what we're going for here. I tried to create conditions to make outperformance likely (small group sizes, much within-subject variation in X relative to between-subject variation, so that Xbar poorly correlates with the group-level confound). Am I missing something in my simulations or is latent Mundlak rarely worth the effort? I'd be glad for any pointers.


## Bernoulli simulations
I started with Richard's simulations of a binary outcome. I was not able to reproduce his model performances exactly. Specifically, the latent Mundlak model does not outperform the non-latent Mundlak model by much, except if you care about `b_Xbar` (i.e. the group-level confound).
...
draws <- bind_rows(
  ...
  brms_latent_mundlak = b_mru_mi %>% gather_draws(b_Y_X, b_Y_Z, bsp_Y_miX2),
 .id = "model") %>% 
...
ggplot(sim, aes(Ug, Xbar, ymin = Xbar - Xse, ymax = Xbar + Xse)) + geom_pointrange(alpha = 0.2)

b_mru <- brm(Y ~ 1 +(1|id) + X + Z + me(Xbar, Xse), data = sim,
            prior = c(
              set_prior("normal(0, 1)", class = "b", coef = "X"),
              set_prior("normal(0, 1)", class = "b", coef = "Z"),
              set_prior("normal(0, 1)", class = "b", coef = "meXbarXse"),
              set_prior("exponential(1)", class = "sd"),
...
```{r}
b_mru_mi <- brm(bf(Y ~ (1|id) + X + Z + mi(X2)) +
             bf(X2 | mi(Xse) ~ (1|id), family = gaussian()), data = sim %>% mutate(Xse = mean(Xse)),
            prior = c(
              set_prior("normal(0, 1)", class = "b", coef = "X", resp = "Y"),
              set_prior("normal(0, 1)", class = "b", coef = "Z", resp = "Y"),
              set_prior("normal(0, 1)", class = "b", coef = "miX2", resp = "Y"),
