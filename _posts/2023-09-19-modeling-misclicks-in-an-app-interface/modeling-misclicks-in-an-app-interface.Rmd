...
1. The fitted daily false positive probabilities need to be aggregated to the user/cycle level before correcting the observed user/cycle rate.
2. For independent daily false positives, this is the complement of observing no false positives on all tracked days.

```{r}
false_positive_any <- function(fp) -expm1(sum(log1p(-fp)))

no_o_users <- daily %>% 
  filter(!feel_ms, !time %in% 10:20) %>% 
  group_by(id, feel_ms) %>% 
  summarise(fp_users = false_positive_any(fp),
...
mean(no_o_users$fp_users, na.rm = T)
