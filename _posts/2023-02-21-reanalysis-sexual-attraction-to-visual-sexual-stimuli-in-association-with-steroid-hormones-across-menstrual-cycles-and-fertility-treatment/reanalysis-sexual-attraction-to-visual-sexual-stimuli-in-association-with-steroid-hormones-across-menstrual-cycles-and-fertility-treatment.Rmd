...
```{r,eval=FALSE}
m1lag <- brm(
  bf(SR_Intercourse ~ mi(logOESTR_lag2) + mi(logPROG_lag2) + cycle + (1|ID)) +
  bf(logOESTR_lag2 | mi() ~ est_estradiol_fc + (1|ID)) +
  bf(logPROG_lag2 | mi() ~ est_progesterone_fc + (1|ID)) +
    set_rescor(FALSE),
  cycles_longer, 
  iter = 8000, 
  inits = 0,
  control = list(adapt_delta = 0.99, max_treedepth = 15),
  prior = c(set_prior("normal(0,1)", class = "b", resp = "SR_Intercourse"),
            set_prior("normal(0,1)", class = "b", resp = "logOESTRlag2"),
            set_prior("normal(0,1)", class = "b", resp = "logPROGlag2"),
            set_prior("normal(0,1)", class = "sd", resp = "SR_Intercourse"),
            set_prior("normal(0,1)", class = "sd", resp = "logOESTRlag2"),
            set_prior("normal(0,1)", class = "sd", resp = "logPROGlag2"))
  )
```
...
```{r}
imputation_formula <- bf(
  SRIntercourse ~ intercept + 
    blogOESTR*(logOESTRlag2) # logOESTRlag2 = the measured hormone two days in the future
  +  blogPROG*(logPROGlag2) + 
    bcycle* cycle,
  intercept ~ 1 + (1 | ID), # varying intercepts
  bcycle ~ 1,
  blogOESTR ~ 1 + (1 | ID), # varying slopes
  blogPROG ~ 1 + (1 | ID),  # varying slopes
  nlf(logOESTRlag2 ~ estestradiolfc), # estimate measured steroid from imputed steroid
  nlf(logPROGlag2 ~ estprogesteronefc),
  nl = TRUE
) +
  gaussian()

cycles_long_nou <- cycles_longer %>% rename_with(~str_replace_all(., "_", ""))
p <- get_prior(imputation_formula, data = cycles_long_nou) %>%
  mutate(
    prior = case_when(
      class == "b" & coef == "Intercept" ~ "normal(0, 1)",
      class == "sd" & coef == "Intercept" ~ "student_t(7, 0, 1)",
...
m1_i_latent <- brm(
  imputation_formula,
  data = cycles_long_nou,
  prior = p,
  iter = 4000,
  control = list(adapt_delta = 0.99),
  file = "brm-fit-latent-impute"
)
...
It worries me that the results of the latent lag model are more like the results of the imputations without lag than of those with lag. So maybe I didn't specify the nonlinear model correctly. But I don't see the mistake yet.

### Bringing it all together
```{r layout="l-body-outset", fig.height = 5, fig.width = 6, fig.cap = "Non-varying slopes for log estradiol and log progesterone"}
draws <- bind_rows(
  latent_impute_lag2 = m1_i_latent %>% gather_draws(`b_blog.+`, regex = T) %>% mutate(.variable = str_replace(str_replace(.variable, "b_b", "b_"), "_Intercept", "")),
