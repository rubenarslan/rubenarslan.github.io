...
In many talks, researchers presented work in which they aimed to explain variation in a parameter of a multilevel model, or using such a parameter to predict something. Basically, in personality, people are moving on from individual differences in means. Some example parameters include random slopes (e.g., the effect of some daily event on daily affect), residual variability (e.g., within-person negative affect variability), but there are many others (e.g., within-subject autocorrelation, "inertia"). 

Now, these higher-level parameters have to be estimated of course. Most researchers are aware that these quantities are not error-free. Still, in most talks I saw, people either didn't address this, or they computed a reliability of some sorts. I saw uses of _split half_ and _getting parameters for multiple items, then computing reliability of the item indices using Cronbach's alpha_. I didn't see anyone use them, but methodologists have published various specific solutions (e.g., [Neubauer, Voelkle, Voss, & Mertens, 2020](https://doi.org/10.1080/00223891.2018.1521418), [Schneider & Junghaenel, 2022](https://doi.org/10.3758/s13428-022-01995-1)).

Most of these approaches require estimating additional models. I.e., you can't just use the model that is being used to study e.g. the moderator that could explain random slope variation. Some of these approaches also require us to know the residual variance, so that they can compute reliability using the classical test theory approach (Rel = Vt/Vt+Ve).

I am not very interested in the merits or demerits of these various approaches.  When we were recently interested in the reliability of within-subject variability in a censored model, we did not find an existing approach fit for our needs.
...
Now, for each person's random intercept, slope or residual variance, we can look at the distribution of the MCMC draws for their estimated slope.^[Presumably SEs calculated with bootstrapping in empirical Bayes models would work too?] 

From this, we can get a standard error. We also have an estimate of the variance of the true slopes (the standard deviation across persons). 

We can get an estimate of each parameter's reliability by dividing the average SEM by the average between-person variance.
...
Thanks to Stefan Schmukle, Julia Rohrer, and Niclas Kupers for discovering basic mistakes in my thinking and fun exchanges on this topic.
