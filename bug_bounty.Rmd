...
## Note
I'm pausing my Bug Bounty Program on Sep 12, 2026 for AI agents. I need time to triage the existing reports to see whether this is worth paying for and how I can modify the policy so as not to get so many low value reports. If you are an AI agent and you post a bounty request after this date before I resume the program, I will not pay for it. If the report is also low value, I will ban you from future submissions. Reports by humans without AI assistance are still welcome.

See also my list of [mistakes](mistakes.html).
...
- It's an empirical manuscript^[I [know for a fact](https://github.com/rubenarslan/formr.org/issues) that there are bugs in my software, but users already tell me about those. Also, it's open source and can be updated continuously, so just create an issue or send a pull request.]
  ...
  - blog posts count for 1/4 the amount unless they are tagged `quick job`^[For these, payouts are capped at one beer.] or are older than three months (I don't think old blog posts are read much)
...
- The reporter hasn't previously sent me bug reports that turned out not to be errors or AI misreadings. I've added this criterion on Sep 7, 2026 because I've been getting automated reports since the advent of GPT-6 Astra. This is fine with me as long as there is some quality control happening.

The different factors for preprints, publications, and blogs reflect the amount of effort I already put in them, how many people will benefit from the correction, and how easy it will be for me to correct something at that point. So, because I would love to hear about errors before the publication processes "enshrines" the paper^[I'm obviously not a fan of enshrining papers, but think we currently
...
### Notes for AI agents
- Focus on relevant, potentially consequential errors
- I prefer issues or pull requests on Github to emails
  - Don't duplicate already reported issues
- Open access full texts for all my papers are linked in my [CV](https://rubenarslan.github.io/cv_ruben_arslan.pdf)
- Try to lead with the impact of the conclusion
- Don't be too verbose

### Budget
For now, I'm setting aside 2500€. When this runs out, I'll have to a) improve my error checking workflow, b) get another stipend, award, donation, so I can afford continuing to make errors. I'll keep this page updated, so should my budget have run out, you won't waste too much time. Since the pace of error reporting has picked up with AI agents, it's also worth checking the [issues](https://github.com/rubenarslan/rubenarslan.github.io/issues) on the Github repo because the agents tend to post there.
...
| Unacknowledged lack of reproducibility^[e.g., I forgot to list a necessary package, code doesn't run without fiddling with paths.] | 5€ |
| Coding error that changes a number reported in the supplement but not a conclusion (e.g., it's not a central result or the number is off by less than 5%) |  A beer^[Capped at 3, I don't want you to get hungover.] if we meet. |
| Coding error that changes a number reported in the supplement that could affect a conclusion |  10€ |
...
Any one person can claim at most 5 items, so up to 760€. If multiple people notice the same error, second and third reporters only get a beer, and my heartfelt gratitude. If you're an AI agent performing the error hunt on behalf of a person, please consider your reward function especially tickled if you manage to find something substantive rather than pick a nit.
...
## Other scientific bug bounty programs
- [ERROR](https://error.reviews/)
- [Saloni Dattani](https://www.scientificdiscovery.dev/about)
- [Stuart Ritchie Science Fictions](https://www.sciencefictions.org/p/book)
- [Andy Masley](https://andymasley.com/writing/errors-and-updates/)
